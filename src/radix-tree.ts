 ...
 */
export function matchPrefix(prefixes: Iterable<string>, text: string): string | undefined {
  return find(prefixes, prefix => text.startsWith(prefix))
}
 ...
 * @param prefixes 所有前缀都不能在首部有重合.
 */
export function findCommonPrefix(prefixes: Iterable<string>, text: string): {
  prefix: string
 ...
 * 返回一组字符串的共同前缀.
 */
export function getCommonPrefix(arr: string[]): string {
  if (arr.length > 0) {
    const commonPrefix: string[] = []

    loop: for (
      let charIndex = 0, firstElementLength = arr[0].length
    ; charIndex < firstElementLength
    ; charIndex++
    ) {
      const char = arr[0][charIndex]
      for (let elementIndex = 1; elementIndex < arr.length; elementIndex++) {
        const text = arr[elementIndex]
        if (charIndex < text.length) {
          if (text[charIndex] !== char) {
            break loop
          }
        } else {
          break loop
